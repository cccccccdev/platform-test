...
import { Button, Form, Modal, Select, Switch, Typography, message } from 'antd';
import { ArrowRightOutlined, DownOutlined, PlusOutlined } from '@ant-design/icons';
import { useNavigate } from 'react-router-dom';
import type { AbilityItem, BusinessTypeItem } from './capabilityDataStore';
import { SERVICE_MODEL_OPTIONS, type ServiceAbilityDirection, type ServiceActionRecord, type ServiceRecord, type ServiceRunModel } from './serviceReferenceData';
...
type ConnectionDraft = { service?: string; ability?: string; direction: ServiceAbilityDirection };

export default function BusinessTypeDemoCanvas({ businessType, services, abilityGroup, connections, focus, onSelect }: {
  businessType: string;
  ...
  const [subOrderTarget, setSubOrderTarget] = useState<string | null>(null);
  const [subOrderDraft, setSubOrderDraft] = useState(false);
  const [enabledSubOrders, setEnabledSubOrders] = useState(() => getEnabledSubOrderModes());
  ...
  const expandedService = (name: string) => Boolean(focus && (focus.kind === 'service'
    ? focus.name === name
    : connections.some((connection) => connection.service === name && connection.ability === focus.name)));
  const expandedAbility = (name: string) => Boolean(focus && (focus.kind === 'ability'
    ? focus.name === name
  ...
  }, [businessType, connections, focus, mappings]);

  useEffect(() => { setHoveredAction(null); setHoveredMappingKey(null); }, [focus]);
  ...
  const openAbilityConfig = (route: 'features' | 'link-state-machine', ability: string) => navigate(`/basic-info/capability/${route}?${new URLSearchParams({ bt: businessType, ability, ...routeQuery('ability', ability) })}`);
  const saveModel = () => {
  ...
  const saveSubOrder = () => {
    if (subOrderTarget && subOrderDraft && !enabledSubOrders.has(getSubOrderModeKey(businessType, subOrderTarget))) {
      setEnabledSubOrders(enableSubOrderMode(businessType, subOrderTarget));
      message.success('Sub-order enabled');
    }
    setSubOrderTarget(null);
  };
  const visibleMapping = (mapping: ActionMapping) => Boolean(focus && (focus.kind === 'service' ? mapping.service === focus.name : mapping.ability === focus.name));
  const openAddMapping = () => setConnectionDraft({
    ...
    direction: 'service-to-ability',
  });
  const saveConnection = () => {
    if (!connectionDraft?.service || !connectionDraft.ability) return;
    const result = addConnection(businessType, connectionDraft.service, connectionDraft.ability, connectionDraft.direction);
    if (result !== 'added') {
  ...
  const openFieldMapping = (mapping: ActionMapping) => navigate(`/basic-info/service/capability/field-mapping?${new URLSearchParams({
    ...
    abilityAction: mapping.abilityAction, direction: mapping.direction,
  ...
  const renderAbility = (ability: AbilityItem) => {
    ...
    const subOrderOn = enabledSubOrders.has(getSubOrderModeKey(businessType, ability.name));
    return <article className={`bt-demo-expand-node ${expanded ? 'expanded' : ''}`} data-demo-kind="ability" data-demo-name={ability.name} key={ability.key}>
      <button type="button" className={`bt-demo-node ${related.length ? 'mapped' : 'unmapped'}`} aria-expanded={expanded} onClick={() => onSelect('ability', ability.name)}>
        <span><span className="bt-demo-node-name">{ability.name}</span><span className="bt-demo-node-meta">{related.length ? `${related.length} mapped ${related.length === 1 ? 'Service' : 'Services'}` : 'No mapping'}</span></span><DownOutlined className="bt-demo-expand-chevron" />
      </button>
      <div className="bt-demo-expand-reveal"><div className="bt-demo-expand-content">
        <div className="bt-demo-expand-ability-tools">
          <button type="button" onClick={() => { setSubOrderTarget(ability.name); setSubOrderDraft(subOrderOn); }}>Sub-order {subOrderOn ? 'ON' : 'OFF'}</button>
          <button type="button" onClick={() => openAbilityConfig('features', ability.name)}>Features</button>
          <button type="button" onClick={() => openAbilityConfig('link-state-machine', ability.name)}>State Machines</button>
        </div>
  ...
  return <>
    <div className="bt-demo-mapping-toolbar"><div className="bt-demo-column-headings"><span>Service</span><span>{focus ? 'Action mapping · click ↗' : 'Service ↔ Ability'}</span><span>Ability</span></div><Button icon={<PlusOutlined />} onClick={openAddMapping} disabled={!services.length || !abilities.length}>Add mapping</Button></div>
      ...
      <div className="bt-demo-connection-canvas bt-demo-inline-canvas" style={{ height: geometry.height }}>
        {connections.length ? <svg viewBox={`0 0 ${geometry.width} ${geometry.height}`} role="img" aria-label={focus ? 'Action mappings' : 'Service and Ability connections'}>
          <defs>
            ...
            <marker id="bt-demo-inline-arrow-start" markerWidth="7" markerHeight="7" refX="1" refY="3.5" orient="auto"><path d="M6 0 L0 3.5 L6 7" fill="none" stroke="currentColor" strokeWidth="1.5" /></marker>
            ...
            <marker id="bt-demo-pair-arrow-start" markerWidth="7" markerHeight="7" refX="1" refY="3.5" orient="auto"><path d="M6 0 L0 3.5 L6 7" fill="none" stroke="#a99ab3" strokeWidth="1.3" /></marker>
          </defs>
          {connections.map((connection, index) => {
            const point = geometry.coarse[index];
            if (!point || point.startY < 0 || point.endY < 0) return null;
            return <path key={point.key} className={`bt-demo-pair-line ${focus ? 'bt-demo-line-hidden' : 'bt-demo-line-visible'}`} d={`M 3 ${point.startY} C ${geometry.width * .38} ${point.startY}, ${geometry.width * .62} ${point.endY}, ${geometry.width - 12} ${point.endY}`} {...(connection.direction === 'ability-to-service' ? { markerStart: 'url(#bt-demo-pair-arrow-start)' } : { markerEnd: 'url(#bt-demo-pair-arrow-end)' })} />;
          })}
          {mappings.map((mapping, index) => {
            ...
            return <g key={point.key} className={`bt-demo-action-connection ${visible ? 'interactive' : ''}`} role={visible ? 'link' : undefined} tabIndex={visible ? 0 : -1}
              aria-label={visible ? `Field Mapping: ${mapping.service} ${mapping.serviceAction} ${mapping.direction === 'ability-to-service' ? 'from' : 'to'} ${mapping.ability} ${mapping.abilityAction}` : undefined}
              ...
              <path className={`bt-demo-action-line ${visible ? highlighted ? 'bt-demo-line-visible' : 'bt-demo-line-dimmed' : 'bt-demo-line-hidden'}`} d={path} {...(mapping.direction === 'ability-to-service' ? { markerStart: 'url(#bt-demo-inline-arrow-start)' } : { markerEnd: 'url(#bt-demo-inline-arrow-end)' })} />
    ...
    <Modal title="Add Service / Ability mapping" open={Boolean(connectionDraft)} onCancel={() => setConnectionDraft(null)} onOk={saveConnection} okText="Add mapping" cancelText="Cancel"
      ...
      <Form labelCol={{ span: 7 }} wrapperCol={{ span: 15 }} colon>
        ...
        <Form.Item label="Direction" required><Select value={connectionDraft?.direction} options={[{ label: 'Service → Ability', value: 'service-to-ability' }, { label: 'Ability → Service', value: 'ability-to-service' }]}
          onChange={(direction: ServiceAbilityDirection) => setConnectionDraft((draft) => draft && ({ ...draft, direction }))} /></Form.Item>
    ...
    <Modal title="Sub-order" open={Boolean(subOrderTarget)} onCancel={() => setSubOrderTarget(null)} onOk={saveSubOrder} okText="Save" cancelText="Cancel"
      okButtonProps={{ disabled: !subOrderDraft || Boolean(subOrderTarget && enabledSubOrders.has(getSubOrderModeKey(businessType, subOrderTarget))) }}
      className="sub-order-mode-modal" width={640}>
      <div className="sub-order-mode-context"><span>Business Type</span><strong>{businessType}</strong><span>Ability</span><strong>{subOrderTarget}</strong></div>
      <div className="sub-order-mode-switch-row"><span>Sub-order Mode</span><Switch checked={subOrderDraft} disabled={Boolean(subOrderTarget && enabledSubOrders.has(getSubOrderModeKey(businessType, subOrderTarget)))} checkedChildren="ON" unCheckedChildren="OFF" onChange={setSubOrderDraft} /></div>
      <p className="sub-order-mode-note">Once enabled, Sub-order Mode cannot be disabled in this Demo.</p>
    </Modal>
  </>;
}
