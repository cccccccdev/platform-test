...
import { Alert, Button, Checkbox, Empty, Form, Input, InputNumber, Modal, Space, Switch, Table, Tooltip, Typography, message } from 'antd';
...
export default function ChannelSharedLines({ lines, references, onChange }: Props) {
  ...
  const save = async () => {
    ...
    if (duplicate) {
      form.setFields([{ name: 'lineName', errors: ['Line Name already exists in the current Party or Account scope.'] }]);
      return;
    }
  ...
  return <>
    <div className="party-line-actions"><Button type="primary" onClick={() => openEditor()}>Add Line</Button></div>
    <Table
      rowKey="id"
      dataSource={lines}
      pagination={false}
      ...
      columns={[
        ...
        { title: 'Operation', width: 190, fixed: 'right' as const, render: (_: unknown, row: SharedLine) => {
          ...
          return <Space size="small">
            ...
            <Tooltip title={activeReferences.length ? 'This Line has enabled Endpoint references in the current scope and cannot be configured.' : undefined}>
    ...
    <Modal title="Line References" open={Boolean(referenceLine)} onCancel={() => setReferenceLine(null)} footer={<Button onClick={() => setReferenceLine(null)}>Close</Button>} width={720}>
      {referenceLine && <>
        ...
        <Alert type="info" showIcon message="Only enabled Endpoint references in the current scope are shown." style={{ marginBottom: 16 }} />
        <Table<LineReference>
          rowKey={(reference) => `${reference.party}:${reference.path}`}
          dataSource={references.filter((reference) => reference.lineId === referenceLine.id)}
          pagination={false}
          locale={{ emptyText: <Empty description="This Line has no enabled Endpoint references." /> }}
          columns={[
