...
export function InboundRequestDrawer({ open, readOnly = false, onClose, onSave }: Props) {
  ...
  return <Drawer title={<Space><span>Configure Inbound Request</span><Tag color="cyan">inboundRequest</Tag></Space>} width="min(1180px, 92vw)" open={open} onClose={onClose} destroyOnClose extra={!readOnly && <Space><Button onClick={onClose}>Cancel</Button><Button type="primary" onClick={() => form.validateFields().then(onSave)}>Save</Button></Space>}>
    <ConfigProvider componentSize="middle" theme={{ components: { Form: { itemMarginBottom: 10 } } }}>
      <Form form={form} disabled={readOnly} layout="vertical" initialValues={{ codeMappingMode: 'default' }}>
        <Alert type="info" showIcon message="Request Message Format is inherited from Match Capability · Inbound Preprocess (A)." description="Define the complete request fields here using the preprocessed, decrypted request. Do not add the encrypted carrier field that was already consumed by A." style={{ marginBottom: 14 }} />
        <Tabs activeKey={activeTab} onChange={setActiveTab} items={[
          { key: 'mapping', label: 'Request Mapping', children: <Tabs size="small" items={[
            { key: 'path', label: 'Path Vars', children: <Form.Item name="pathVariables" initialValue={[]}><FlatFieldMappingEditor direction="response" title="Path Variable · API to SPI" addLabel="Add Variable" fieldPlaceholder="External field" sourceOptions={[]} targetOptions={spiRequestOptions} dataTypeOptions={flatTypes} operationOptions={mappingOperationOptions} targetPlaceholder="SPI request field" /></Form.Item> },
            { key: 'query', label: 'Params', children: <Form.Item name="queryParameters" initialValue={[]}><FlatFieldMappingEditor direction="response" title="Query Parameter · API to SPI" addLabel="Add Parameter" fieldPlaceholder="External field" sourceOptions={[]} targetOptions={spiRequestOptions} dataTypeOptions={flatTypes} operationOptions={mappingOperationOptions} targetPlaceholder="SPI request field" /></Form.Item> },
            { key: 'header', label: 'Headers', children: <Form.Item name="requestHeaders" initialValue={[]}><FlatFieldMappingEditor direction="response" title="Request Header · API to SPI" addLabel="Add Header" fieldPlaceholder="External field" sourceOptions={[]} targetOptions={spiRequestOptions} dataTypeOptions={flatTypes} operationOptions={mappingOperationOptions} targetPlaceholder="SPI request field" /></Form.Item> },
            { key: 'body', label: 'Body', children: <Form.Item name="requestBody" initialValue={[]}><BodySchemaMappingEditor direction="response" sourceOptions={[]} targetOptions={spiRequestOptions} dataTypeOptions={types} operationOptions={mappingOperationOptions} targetPlaceholder="SPI request field" /></Form.Item> },
            { key: 'script', label: 'Custom Script', children: <GroovyScriptEditor name="requestMappingScript" helpText="Map pathVariables, queryParameters, requestHeaders and the preprocessed requestBody to SPI Request." /> },
          ]} /> },
...
export function InboundResponseDrawer({ open, readOnly = false, onClose, onSave }: Props) {
  ...
  return <Drawer title={<Space><span>Configure Inbound Response</span><Tag color="green">inboundResponse</Tag></Space>} width="min(1180px, 92vw)" open={open} onClose={onClose} destroyOnClose extra={!readOnly && <Space><Button onClick={onClose}>Cancel</Button><Button type="primary" onClick={() => form.validateFields().then(onSave)}>Save</Button></Space>}>
    <ConfigProvider componentSize="middle" theme={{ components: { Form: { itemMarginBottom: 10 } } }}>
      <Form form={form} disabled={readOnly} layout="vertical" initialValues={{ responseFormat: 'JSON' }}>
        <Tabs items={[
          { key: 'mapping', label: 'Response Mapping', children: <Tabs size="small" items={[
            { key: 'header', label: 'Headers', children: <Form.Item name="responseHeaders" initialValue={[]}><FlatFieldMappingEditor title="Response Header · SPI to API" addLabel="Add Header" fieldPlaceholder="External response field" sourceOptions={spiResponseOptions} dataTypeOptions={flatTypes} operationOptions={mappingOperationOptions} sourcePlaceholder="SPI response field" /></Form.Item> },
            { key: 'body', label: 'Body', children: <><Form.Item name="responseFormat" label="Response Message Format" rules={[{ required: true }]}><Select options={formats} /></Form.Item><Form.Item name="responseBody" initialValue={[]}><BodySchemaMappingEditor sourceOptions={spiResponseOptions} dataTypeOptions={types} operationOptions={mappingOperationOptions} sourcePlaceholder="SPI response field" /></Form.Item></> },
            { key: 'script', label: 'Custom Script', children: <GroovyScriptEditor name="responseMappingScript" helpText="Map SPI Response and Flow Context to responseHeaders and responseBody." /> },
          ]} /> },
          { key: 'security', label: 'Security', children: <Card size="small"><div style={{ border: '1px solid #f0f0f0', borderRadius: 6, overflow: 'hidden' }}>
