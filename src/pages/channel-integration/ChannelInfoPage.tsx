...
import ChannelInfoCapabilityPage from './ChannelInfoCapabilityPage';
import ChannelInfoServiceChannelPage from './ChannelInfoServiceChannelPage';
import ChannelInfoPartyPage from './ChannelInfoPartyPage';
import ChannelInfoLinesPage from './ChannelInfoLinesPage';
import { channelInfoPartyTitle } from './channelInfoPartyRoute';

const { Content, Sider } = Layout;
const { Text } = Typography;

type MainState = 'INIT' | 'PENDING' | 'TO_BE_VERIFY' | 'SUCCESS' | 'FAIL';
type PageKey = 'capability' | 'lines' | 'party' | 'external-internal' | 'internal-external' | 'institution' | 'chain' | 'asset-route' | 'requery' | 'runtime-route-matching' | 'runtime-flow-groups' | 'service-channel';
...
const pageKeyFromPath = (pathname: string): PageKey => {
  ...
  if (normalizedPath.includes('/channel-info/lines')) return 'lines';
  if (normalizedPath.includes('/channel-info/party')) return 'party';
  ...
  if (normalizedPath.includes('/channel-info/service-channel')) return 'service-channel';
  if (
...
export default function ChannelInfoPage() {
  ...
  const [serviceChannelDetailCode, setServiceChannelDetailCode] = useState<string | null>(null);
  ...
  const isLines = pageKey === 'lines';
  const isParty = pageKey === 'party';
  ...
  const isServiceChannel = pageKey === 'service-channel';

  useEffect(() => {
    const nextPageKey = pageKeyFromPath(location.pathname);
  ...
  const renderActivePage = () => {
    ...
    if (isLines && applied) return <ChannelInfoLinesPage channelCode={channelCode} cloud={applied.cloud} env={applied.env} />;
    if (isParty && applied) return <ChannelInfoPartyPage channelCode={channelCode} cloud={applied.cloud} env={applied.env} routeMatchingEndpoints={routeMatchingEndpoints} />;
    ...
    if (isServiceChannel && applied) return <ChannelInfoServiceChannelPage channelCode={channelCode} cloud={applied.cloud} env={applied.env} configuredAbilities={flowGroupAbilities} selectedCode={serviceChannelDetailCode} onSelect={setServiceChannelDetailCode} />;
  ...
  const title = runtimeDetailView?.kind === 'route-matching'
    ...
    : detailView?.type === 'approval'
      ? 'Approval Detail'
    : isCapabilityFeatures ? 'Features' : isCapability ? 'Capability' : isLines ? 'Lines' : isParty ? channelInfoPartyTitle(location.pathname) : isExternal ? 'External->Internal' : isInternal ? 'Internal->External' : isInstitution ? 'Institution' : isChain ? 'Chain' : isAssetRoute ? 'Asset Route' : isRequery ? 'Requery Strategy' : isRuntimeRouteMatching ? 'Route Matching' : isServiceChannel && serviceChannelDetailCode ? 'Service Channel Capability' : isServiceChannel ? 'Service Channel' : 'Flow Groups';
  ...
  return (
    <Layout className="integration-shell">
      <Sider width={200} theme="dark" className="integration-sidebar channel-info-sidebar">
        ...
        {runtimeDetailView ? (
          ...
          <Menu
            ...
            onClick={(item) => {
              if (item.key === 'capability' || item.key === 'lines' || item.key === 'party' || item.key === 'external-internal' || item.key === 'internal-external' || item.key === 'institution' || item.key === 'chain' || item.key === 'asset-route' || item.key === 'requery' || item.key === 'runtime-route-matching' || item.key === 'runtime-flow-groups' || item.key === 'service-channel') {
                setPageKey(item.key);
                setServiceChannelDetailCode(null);
                resetForm();
                resetRequeryForm();
                ...
                if (item.key === 'service-channel') navigate(`/channel-integration/${channelCode}/channel-info/service-channel`);
                if (item.key === 'lines') navigate(`/channel-integration/${channelCode}/channel-info/lines`);
                if (item.key === 'party') navigate(`/channel-integration/${channelCode}/channel-info/party`);
                if (item.key === 'capability') navigate(`/channel-integration/${channelCode}/channel-info`);
                if (item.key !== 'lines' && item.key !== 'party' && item.key !== 'institution' && item.key !== 'chain' && item.key !== 'asset-route' && item.key !== 'runtime-route-matching' && item.key !== 'runtime-flow-groups' && item.key !== 'service-channel') navigate(`/channel-integration/${channelCode}/channel-info`);
              }
            }}
            items={[
      ...
      <Layout className="integration-main">
        <div className="channel-info-topbar">
          <Space size={18} align="center" wrap>
            ...
            <Button type="primary" onClick={() => {
              if (!cloud || !env) { message.warning('Please select Cloud and Env first'); return; }
              setServiceChannelDetailCode(null);
              setApplied({ cloud, env });
            }}>Apply</Button>
          </Space>
          <UserProfile />
        </div>
        <Content style={{ padding: 24 }}>
          <Breadcrumb items={[{ title: channelCode }, ...(runtimeDetailView ? [{ title: 'Runtime Control' }, { title: runtimeDetailView.kind === 'route-matching' ? 'Route Matching' : 'Flow Groups' }, { title }] : isCapabilityFeatures ? [{ title: 'Capability' }, { title: 'Features' }] : isCapability ? [{ title: 'Capability' }] : isLines ? [{ title: 'Lines' }] : isParty ? [{ title: 'Party' }, ...(title !== 'Party' ? [{ title }] : [])] : isInstitution ? [{ title: 'Institution' }] : isChain ? [{ title: 'Chain' }] : isAssetRoute ? [{ title: 'Asset Route' }] : isRequery ? [{ title: 'Requery Strategy' }] : isServiceChannel ? [{ title: 'Service Channel' }] : isRuntimeRouteMatching || isRuntimeFlowGroups ? [{ title: 'Runtime Control' }, { title }] : [{ title: 'Response Code' }, { title: detailView ? 'External->Internal' : title }, ...(detailView ? [{ title }] : [])])]} style={{ marginBottom: 12 }} />
